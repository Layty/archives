...
#define UPLOAD_FILE "/var/cwmp_client.pem"
...
int upload(char *filename, char *name, int offset, int len)
{
  ...
  sprintf(cmd, "nvram_set config2 TR_CLIENT_CERT_FILE %s", tar_file_name);
  ...
  sprintf(cmd, "nvram_set tr069cert TR069_CC \"%s\"", tmpdata);
	system(cmd);
	memset(cmd, 0, CMD_SIZE);

  sprintf(cmd, "ralink_init gen tr069_cc_cert");
	system(cmd);
...
<body>\n<h1> Certificate Install</h1>\n");
	...
	if (!upload(UPLOAD_FILE, name, file_begin, file_end - file_begin))
	{
