...
/*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
/*BEGIN:Added by huangcd for T5214EWL_vth_perf_opt*/
#include <linux/proc_fs.h>

typedef struct _PSEUDO_ADAPTER_VTHX {
    struct net_device *RaethDev;
    struct net_device *PseudoDev;
    struct net_device_stats stat;
	unsigned short vlan_id;    
}PSEUDO_ADAPTER_VTHX, PPSEUDO_ADAPTER_VTHX;

#define MAX_PSEUDO_ENTRY_VTHX      5
#define DEV_NAME_VTHX          "vth"
#define DEV_NAME_WVTH0          "wvth0"

int VirtualIF_ioctl_vthx(struct net_device * net_dev, struct ifreq * ifr, int cmd);
struct net_device_stats *VirtualIF_get_stats_vthx(struct net_device *dev);
int VirtualIF_open_vthx(struct net_device * dev);
int VirtualIF_close_vthx(struct net_device * dev);
int VirtualIFSendPackets_vthx(struct sk_buff * pSkb, struct net_device * dev);
void virtif_setup_statistics_vthx(PSEUDO_ADAPTER_VTHX * pAd);
static int ei_set_mac2_addr_vthx(struct net_device *dev, void *p);
void RAETH_Init_PSEUDO_vthx(pEND_DEVICE pAd, struct net_device *net_dev);
/*END:  Added by huangcd for T5214EWL_vth_perf_opt*/

/*BEGIN:Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/
int raeth_pktct = 0;
unsigned int reskb_inque_succ = 0;
unsigned int reskb_inque_fail = 0;
unsigned int reskb_deque_succ = 0;
unsigned int reskb_deque_fail = 0;

static int raeth_pktct_read_proc(char* page, char ** start, off_t off, int count, 
		int* eof, void * data) 
{
	int n = 0;
	char *p = page;							

	n = sprintf(p,"raeth_pktct: %u\n",raeth_pktct);
	p += n;

	n = sprintf(p,"reskb_inque_succ: %u\n",reskb_inque_succ);
	p += n;

	n = sprintf(p,"reskb_inque_fail: %u\n",reskb_inque_fail);
	p += n;

	n = sprintf(p,"reskb_deque_succ: %u\n",reskb_deque_succ);
	p += n;    

	n = sprintf(p,"reskb_deque_fail: %u\n",reskb_deque_fail);
	p += n; 
...
static int raeth_pktct_write_proc(struct file* file, const char* buffer, 
		unsigned long count, void *data) 
{
	...
	if(!foo)
    {
		if (net_ratelimit()) 
        {      
			printk(KERN_ERR "raeth_pktct_write_proc: out of memory.\n");
        }
		return count;
	}
...
static void raeth_pktct_init_proc(void)
{
	struct proc_dir_entry* entry = NULL;

	entry = create_proc_entry("raeth_pktct", 0644, proc_net);    
	entry->read_proc  = raeth_pktct_read_proc;
	entry->write_proc = raeth_pktct_write_proc;

    return;
}

static void raeth_pktct_cleanup_proc(void)
{
	remove_proc_entry("raeth_pktct", proc_net);

    return;
}
/*END:   Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/  
/*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
...
#endif
{
	...
	for ( ; ; ) {
        ...
        /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
		raeth_pktct++;  /*Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/  
        /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/		

        /*BEGIN:Modified by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
		/*BEGIN:Deleted by huangcd for T5214EWL_vth_perf_opt*/
		#if 0
        #ifdef CONFIG_PSEUDO_SUPPORT
        ...
        #endif
		#endif
		/*END:Deleted by huangcd for T5214EWL_vth_perf_opt*/
        /*END:  Modified by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/					
		...
		if((my_veth->h_vlan_proto & 0xFF) == 0x81) 
		{
     		special_tag = 1;
		    port_num = ((my_veth->h_vlan_proto >> 8) & 0xF);
			...
			else if((1 <= port_num) && (port_num <= 4))
			{
                
			    pkt_to_dev = ei_local->vth[port_num - 1];
			}
            #endif
		...
		if (unlikely(skb==NULL)) 
        {
            ...
            /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
            reskb_deque_fail++;  /*Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/
            /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/				
        ...
        {
            /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
            reskb_deque_succ++;  /*Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/
            /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/			
        }
...
#endif // defined(CONFIG_RALINK_RT6855) || defined(CONFIG_RALINK_RT63365)//
...
#define MIN_PKT_LEN  64
	...
	if(((ei_local->skb_free[tx_cpu_owner_idx]) ==0) && (ei_local->skb_free[tx_cpu_owner_idx_next]==0)){
		...
		if(ei_local->skb_free[tx_cpu_owner_idx_next2]!=0){
                ...
                /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
				/*BEGIN:Added by huangcd for T5214EWL_vth_perf_opt*/
				netif_stop_queue(ei_local->wvth);	
				
				netif_stop_queue(ei_local->vth[0]);
				netif_stop_queue(ei_local->vth[1]);
				netif_stop_queue(ei_local->vth[2]);
				netif_stop_queue(ei_local->vth[3]);
				/*END:	Added by huangcd for T5214EWL_vth_perf_opt*/	
                /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
...
int ei_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    /*BEGIN:Added by huangcd for support set-phydown-via-ioctl(sync from t8014ews-proj),2014.03.19*/
	struct eth_diag etd;
    /*END:  Added by huangcd for support set-phydown-via-ioctl(sync from t8014ews-proj),2014.03.19*/	
...
#define _ESW_REG(x)	(*((volatile u32 *)(RALINK_ETH_SW_BASE + x)))
        ...
        /*BEGIN:Added by huangcd for support set-phydown-via-ioctl(sync from t8014ews-proj),2014.03.19*/
		case RAETH_ESW_GET_INFO:
		{
			u32 stat_curr;			
			copy_from_user(&etd , ifr->ifr_data, sizeof(etd ));
          
			if(etd.ed_u.portnum < 5)
			{
				/*datasheet page 103: POA: Port Ability (offset: 0x80)*/
				stat_curr = *((volatile u32 *)(RALINK_ETH_SW_BASE+0x80));

				if (etd.val == 1)
				{
					etd.val = 0x1&(stat_curr >> (9 + etd.ed_u.portnum));
				}
				else if (etd.val == 2)
				{			
					etd.val = 0x1&(stat_curr >> (25 + etd.ed_u.portnum));
					//printk("port status = %x\n",etd.val);
				}
				else if (etd.val == 3)
				{				
					etd.val = 0x1&(stat_curr >> (0 + etd.ed_u.portnum));
				}					
				else
				{				
					etd.val = -1;
			...
			copy_to_user(ifr->ifr_data,&etd,sizeof(etd));
			
			break;
		}
		case RAETH_ESW_SET_INFO:
		{
			u32 reg_val;
			u32 port_enable = 0;
			...
			if(etd.ed_u.portnum < 5)
			{
				/*datasheet page 105:POC1: Port Control 0 (offset: 0x90)*/
				reg_val = *((volatile u32 *)(RALINK_ETH_SW_BASE+0x90));
               
				/*if the reg bitmap is 0,the port is enable, otherwise if disable*/
				if(port_enable == 0)
				{
					reg_val |= (1 << (23 + etd.ed_u.portnum));
				}
				else if(port_enable == 1)
				{
					reg_val &= ~(1 << (23 + etd.ed_u.portnum));
				}

				sysRegWrite(RALINK_ETH_SW_BASE+0x90, reg_val);
			}
			else
			{
				return -EOPNOTSUPP;
	...
	if ((ei_local->skb_free[skb_free_idx]) != 0 && tx_desc[skb_free_idx].txd_info2.DDONE_bit==1) 
    {
		while(tx_desc[skb_free_idx].txd_info2.DDONE_bit==1 && (ei_local->skb_free[skb_free_idx])!=0 )
        {
    	    ...
    	    if (skb_queue_len(&ei_local->rx0_recycle) < NUM_RX_DESC && skb_recycle_check(ei_local->skb_free[skb_free_idx], MAX_RX_LENGTH )) 
            {
                ...
                /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
                reskb_inque_succ++;  /*Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/
                /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/				
	        }
            else
            #endif
            {          
                ...
                /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
                reskb_inque_fail++;  /*Added by huangcd for mantis0001472(L2-L3-performance-opt,skb-recycle),2015.02.10*/
                /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/					
            }
            #endif
		...
		/*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
		/*BEGIN:Added by huangcd for T5214EWL_vth_perf_opt*/			
		netif_wake_queue(ei_local->wvth);

		netif_wake_queue(ei_local->vth[0]);
		netif_wake_queue(ei_local->vth[1]);		
		netif_wake_queue(ei_local->vth[2]);		
		netif_wake_queue(ei_local->vth[3]);
		/*END:	Added by huangcd for T5214EWL_vth_perf_opt*/			
		/*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
...
int ei_open(struct net_device *dev)
{
	...
	*((volatile u32 *)(ESW_IMR)) &= ~(ESW_INT_ALL);
	...
	VirtualIF_open_vthx(ei_local->wvth);
    /*END:  Added by huangcd for T5214EWL_vth_perf_opt*/		
    /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
...
int ei_close(struct net_device *dev)
{
    ...
    /*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
	/*BEGIN:Added by huangcd for T5214EWL_vth_perf_opt*/
	VirtualIF_close_vthx(ei_local->vth[0]);
	VirtualIF_close_vthx(ei_local->vth[1]);
	VirtualIF_close_vthx(ei_local->vth[2]);
	VirtualIF_close_vthx(ei_local->vth[3]);
	
	VirtualIF_close_vthx(ei_local->wvth);
	/*END:	Added by huangcd for T5214EWL_vth_perf_opt*/	
    /*END:  Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/	
...
#elif defined (CONFIG_RT63365_ASIC)
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x3600) = 0x5e33b;//CPU Port6 Force Link 1G, FC ON
    *(unsigned long *)(RALINK_ETH_SW_BASE+0x0010) = 0xffffffe0;//Set Port6 CPU Port
...
#if defined (CONFIG_RT63365_ASIC)
...
{
	u32 phy_val=0;
#if  defined (RT63365_FPGA_BOARD) || defined (CONFIG_RT6855_FPGA) || defined (CONFIG_RT6352_FPGA)
	u32 i=0;
#endif
...
/*BEGIN:Added by huangcd for mantis0001208(lanwan_pingok_for_0.8s_when_bootup), 2015.01.19*/
void LANWANPartition(void)
{
	#if defined (CONFIG_RT5350_ASIC)
	//now all cvn's RT5350-chip product is LLLLW():
	//T5214EWL,T5214EWL-OM,R603W,R603W_SD,T80XXEWL,T8014EWA
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE + 0x0040) = 0x1001; //PVID
	*(unsigned long *)(RALINK_ETH_SW_BASE + 0x0044) = 0x1001; //PVID
	*(unsigned long *)(RALINK_ETH_SW_BASE + 0x0048) = 0x1002; //PVID
	*(unsigned long *)(RALINK_ETH_SW_BASE + 0x0070) = 0xffff506f; //VLAN member
...
/*END:  Added by huangcd for mantis0001208(lanwan_pingok_for_0.8s_when_bootup), 2015.01.19*/
...
void fe_sw_init(void)
{
	...
	// Case3: RT305x/RT335x/RT6855/RT63365 + EmbeddedSW
#if defined (CONFIG_RT_3052_ESW)
...
/*BEGIN:Added by huangcd for mantis0001639(sync_L2L3pref_opt_from_OCN),2015.05.06*/
/*BEGIN:Added by huangcd for T5214EWL_vth_perf_opt*/
int VirtualIF_ioctl_vthx(struct net_device * net_dev,
		    struct ifreq * ifr, int cmd)
{
	/*do nothing*/
...
struct net_device_stats *VirtualIF_get_stats_vthx(struct net_device *dev)
{
	PSEUDO_ADAPTER_VTHX *pAd = netdev_priv(dev);
...
int VirtualIF_open_vthx(struct net_device * dev)
{
    PSEUDO_ADAPTER_VTHX *pPesueoAd = netdev_priv(dev);

    //printk("%s: ===> VirtualIF_open_vthx\n", dev->name);
    netif_start_queue(pPesueoAd->PseudoDev);
...
int VirtualIF_close_vthx(struct net_device * dev)
{
    ...
    //printk("%s: ===> VirtualIF_close_vthx\n", dev->name);
    netif_stop_queue(pPesueoAd->PseudoDev);

    return 0;
}

int VirtualIFSendPackets_vthx(struct sk_buff * pSkb,
			 struct net_device * dev)
{
    PSEUDO_ADAPTER_VTHX *pPesueoAd = netdev_priv(dev);
    END_DEVICE *ei_local;

    //printk("VirtualIFSendPackets_vthx --->\n");
	...
	struct net_device_stats *stats = VirtualIF_get_stats_vthx(dev);
	stats->tx_packets++;
	stats->tx_bytes += pSkb->len; 
	
	pSkb = __vlan_put_tag(pSkb, pPesueoAd->vlan_id);
	if (!pSkb) 
	{
		if(net_ratelimit())
		{
			printk("VirtualIFSendPackets_vthx drop packet.\n");
		}
		stats->tx_dropped++;
		return 0;
	}
    ...
    pSkb->dev = pPesueoAd->RaethDev;
    ei_start_xmit(pSkb, pPesueoAd->RaethDev, 1);	
...
void RAETH_Init_PSEUDO_vthx(pEND_DEVICE pAd, struct net_device *net_dev)
{
    ...
    for (index = 0; index < MAX_PSEUDO_ENTRY_VTHX; index++) 
	{
		...
		if (4 == index)
		{    
    		sprintf(ifname, "%s", DEV_NAME_WVTH0);
		}
		else
		{
		...
		{	
    		sprintf(ifname, "%s%d", DEV_NAME_VTHX, (index - 1));
		}        
        #endif
		...
		if (i < 0 || (memcmp(addr.sa_data, zero1, 6) == 0) ||
		    (memcmp(addr.sa_data, zero2, 6) == 0)) 
		{
			unsigned char mac_addr012345[6] = {0x00, 0x0C, 0x43, 0x28, 0x80, 0x00};
			memcpy(addr.sa_data, mac_addr012345, 6);
		}
