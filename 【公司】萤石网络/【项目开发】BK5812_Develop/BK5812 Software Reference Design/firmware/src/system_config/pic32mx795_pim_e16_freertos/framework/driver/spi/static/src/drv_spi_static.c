...
void DRV_SPI0_BufferAddWriteD(const void * txBuffer, uint32_t size) {
    int32_t txcounter = 0;
    uint8_t data = 0;
    data = ((uint8_t*) txBuffer)[txcounter];
    while (size) {
        //BSP_LEDToggle(BSP_LED_9);
        //if (!SPI_TransmitBufferIsFull_Default(SPI_ID_1)) {
            //BSP_LEDToggle(BSP_LED_9);
            SPI_BufferWrite_Default(SPI_ID_1, data);
            txcounter++;
            size--;
            data = ((uint8_t*) txBuffer)[txcounter];
            //BSP_LEDToggle(BSP_LED_9);
            while (!SPI_ReceiverBufferIsFull_Default(SPI_ID_1));
            //BSP_LEDToggle(BSP_LED_9);
            SPI_BufferRead_Default(SPI_ID_1);
        //}
    }
    return;
}

void DRV_SPI0_BufferAddReadD(const void * rxBuffer, uint32_t size) {
    int32_t rxcounter = 0;
    uint8_t data = 0;
    while (size) {
        //if (!SPI_TransmitBufferIsFull_Default(SPI_ID_1)) {
            SPI_BufferWrite_Default(SPI_ID_1, 0);
            if(rxcounter){
            ((uint8_t*) rxBuffer)[rxcounter-1] = data;
            }
            rxcounter++;
            size--;
            while (!SPI_ReceiverBufferIsFull_Default(SPI_ID_1));
            data = SPI_BufferRead_Default(SPI_ID_1);
        //}
    }
    ((uint8_t*) rxBuffer)[rxcounter-1] = data;
    return;
