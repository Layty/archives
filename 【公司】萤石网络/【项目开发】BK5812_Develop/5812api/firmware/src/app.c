...
const char *string1 = "=== BEKEN 5813 APP UART CMD. ===\r\n";
const char *string2 = "System initial OK!  Stone(@_@) \r\n";
...
void APP_Tasks(void) {
    ...
    switch (appData.state) {
        case USART_ENABLE:
            ...
            memset(uart_msg_buf, 0, sizeof (uart_msg_buf));
            while (WriteString() == false);
            sprintf(uart_msg_buf, "ver. %s\r\n", __DATE__);
        ...
        case USART_TRANSMIT_FIRST_STRING:            
            if (true == WriteString()) {
                DRV_TMR0_Start();
                initFSKReg(FSK_5G_4M);
                rf_task_init(RF_MASTER, rfStatus.rate);
