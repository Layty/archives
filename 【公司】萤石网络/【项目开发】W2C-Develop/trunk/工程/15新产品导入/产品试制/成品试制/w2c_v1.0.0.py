...
class TestFrame(wx.Frame):
    ...
    def OnRssi2g(self,event):  
        ...
        for i in range(len(macAddr)):
            if macAddr[i].isalpha() and macAddr[i].islower():
                macAddr = macAddr[:i] + macAddr[i].upper() + macAddr[i+1:]
    ...
    def checkDevPara(self, event):  
        ...
        hik_hw_mac=hw_infos[4]
        if True == ret:
            if snCode not in hik_hw_sncode or checkCode not in hik_hw_checkcode or eth0_mac not in hik_hw_mac:
