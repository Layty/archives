...
class TestThread(Thread): 
     ...
     def __init__(self, com, sn, check, type, mac): 
     ...
     def run(self): 
          ...
          term.write('tftp 0x80060000 openwrt-ar71xx-ap147-16M-qca-legacy-uboot.bin\x0D')
          time.sleep(3)
          line = term.read(term.inWaiting())
          print line
          if 'done' not in line:
               self.postMsg(1, u"升级uboot失败，请检查网络连接是否正常或者重试！")
               return False
          ...
          term.write('tftp 0x80060000 openwrt-ar71xx-generic-ap147-16M-kernel.bin\x0D')
          time.sleep(4)
          line = term.read(term.inWaiting())
          print line
          if 'done' not in line:
               ...
               if tempSec > waitSec:
                    self.postMsg(1, u"升级kerne失败，请检查网络连接是否正常或者重试！！")
                    return False
          ...
          term.write('tftp 0x80060000 openwrt-ar71xx-generic-ap147-16M-rootfs-squashfs.bin\x0D')
          time.sleep(15)
          line = term.read(term.inWaiting())
          print line
          if 'done' not in line:
               ...
               if tempSec > waitSec:
                    self.postMsg(1, u"升级rootfs失败，请检查网络连接是否正常或者重试！！")
                    return False
          ...
          term.write('uci set wireless.@wifi-iface[0].ssid=\'%s\'\x0D'%self.router2gSSID)
          time.sleep(0.5)
          print term.read(term.inWaiting())

          term.write('uci set wireless.@wifi-iface[0].disabled=\'0\'\x0D')
          time.sleep(0.5)
          print term.read(term.inWaiting())

          term.write('uci set wireless.@wifi-iface[0].encryption=\'none\'\x0D')
          time.sleep(0.5)
          print term.read(term.inWaiting())

          term.write('uci commit\x0D')
          time.sleep(0.5)
          print term.read(term.inWaiting())

          term.write('uci set hik_cfg.admin.passwd==\'%s\'\x0D'%self.routerPassword)
          time.sleep(0.5)
          print term.read(term.inWaiting())

          term.write('uci commit hik_cfg\x0D')
          time.sleep(0.5)
          print term.read(term.inWaiting())
     ...
     def DoWritew2shw(self, seri):
          ...
          '''
          seri.write('nvram set hik_hw_sncode=%s\x0D'%self.hik_hw_sncode)
          print self.hik_hw_sncode
          time.sleep(0.1)


          seri.write('nvram set hik_hw_checkcode=%s\x0D'%self.hik_hw_checkcode)
          print self.hik_hw_checkcode
          time.sleep(0.1)


          seri.write('nvram set hik_hw_type=%s\x0D'%self.hik_hw_type)
          print self.hik_hw_type
          time.sleep(0.1)

          for capa in self.typeCapabilitySet[self.typeList.index(self.hik_hw_type)][1]:
               nvramCmd = 'nvram set %s'%capa
               seri.write('%s\x0D'%nvramCmd)
               print nvramCmd
               time.sleep(0.1)
...
class TestFrame(wx.Frame):
     ...
     def __init__(self, parent):
          ...
          '''
          self.lable8 = wx.StaticText(panel, -1, u"请输入升级包名字:",(10, 140),size = (100,20))
     ...
     def OnBinText(self, event):
        ...
        file = open("w2s_bin.txt", "w+")
        file.truncate()
        file.write(binName)
        file.close
     
     def loadBinName(self):
        if os.path.exists(r'w2s_bin.txt') == True:
            file = open("w2s_bin.txt", "r")
     ...
     def OnUpg(self, event):
          ...
          binName = self.binText.GetValue()
          if len(binName) == 0:
               self.lable7.SetLabel(u"请输入升级包的名字")
               self.upgButton.SetBackgroundColour("Red")
               return False
          '''                 
          TestThread(self.serialPort, hw_infos[1], hw_infos[2], hw_infos[3], hw_infos[4])
